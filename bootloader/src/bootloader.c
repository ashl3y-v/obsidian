...
int main(void) {
  ...
  // A 'reset' on UART0 will re-start this code at the top of main, won't clear
  // flash, but will clean ram.
  ...
  load_initial_firmware(); // note the short-circuit behavior in this function,
                           // it doesn't finish running on reset!
...
void load_initial_firmware(void) {
   ...
   * firmware fully uses the last page, the release message simply is written to
   * a new page.
   */
  ...
  if (rem_fw_bytes == 0) {
    ...
    // If there are more bytes, program them directly from the release message
    // string
    if (rem_msg_bytes > 0) {
...
void load_firmware(void) {
  ...
  // Create 32 bit word for flash programming, version is at lower address, size
  // is at higher address
